...
function ChapterShow() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#8231D3] to-[#6A28AC] relative overflow-hidden">
      ...
      <div className="px-4 py-2 max-w-xl mx-auto relative">
        {chapterById.map((chapter, index) => (
          <div 
            key={chapter.id} 
            className={`relative flex flex-col items-center text-center gap-5 mb-10 ${index % 2 !== 0 ? 'left-0 ml-48' : 'right-0 mr-48'} after:content-[''] after:absolute after:top-full after:w-1 after:h-36 after:border-l-2 after:border-dashed after:border-yellow-300/60 after:rounded-tl-lg`}
            onClick={() => handleChapter(chapter.id)}
          >
            <h2 className="text-lg font-semibold text-white max-w-40">{chapter.title}</h2>
            <div className="w-20 h-20 bg-[#9333ea] rounded-full flex items-center justify-center relative">
